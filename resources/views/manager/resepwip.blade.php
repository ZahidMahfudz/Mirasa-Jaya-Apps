                        ...
                        <div class="mb-3">
                            ...
                            <div id="bahan-container">
                                <div class="input-group mb-2">
                                    ...
                                    <input type="number" name="jumlah_bahan_gr[]" class="form-control" placeholder="Jumlah (gr)" step="0.001" required>
                                    <input type="number" name="jumlah_bahan_kg[]" class="form-control" placeholder="Jumlah (kg)" step="0.001"required>
                                    <input type="number" name="jumlah_bahan_zak[]" class="form-control" placeholder="Jumlah (zak)" step="0.001" required>
            ...
            @foreach ($resep as $index => $resepItem)
                @foreach ($resepItem->bahan_resep as $bahanIndex => $bahanResep)
                    <tr>
                        ...
                        <td>{{ $bahanResep->jumlah_bahan_gr }}</td>
                        <td>{{ $bahanResep->jumlah_bahan_kg }}</td>
                        <td>{{ $bahanResep->jumlah_bahan_zak }}</td>
                        @if ($bahanIndex === 0)
                            <td rowspan="{{ $resepItem->bahan_resep->count() + 1 }}">
                                <button class="btn btn-success" data-bs-toggle="modal"
                                    data-bs-target="#editModal{{ $resepItem->id }}">Edit</button>
                                <form action="{{ route('resepdestroy', $resepItem->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus resep ini?');">
                                    @csrf
