<x-layout-admin>
    <x-slot:title>Gaji Karyawan Borongan Spet</x-slot>
    <x-slot:tabs>Admin-Gaji Karyawan Spet</x-slot>

    <div>
        <a href="/GenerateGajianSpet" class="btn btn-outline-primary">Generate Gaji Untuk Semua Karyawan Borongan</a>
        ...
        <a href="/cetakSlipGajiSpet/{{ $tanggalTerakhir }}" class="btn btn-outline-primary" target="blank_">Cetak Slip Gaji {{ $tanggalTerakhir }}</a>
    </div>
            ...
            @foreach ($gajian_karyawan_borongan as $item)
                ...
                <tr>
                    ...
                    <td rowspan="4" style="text-align: center;">
                        <button class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#editGajian-{{ $item->id_gaji_borongan }}">Edit</button>
                        <button class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#hapusGaji-{{ $item->id_gaji_borongan }}">Hapus</button>

                        <!-- Modal Hapus Gaji -->
                        <div class="modal fade" id="hapusGaji-{{ $item->id_gaji_borongan }}" tabindex="-1" aria-labelledby="hapusGajiLabel" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        Apakah Anda yakin ingin menghapus gaji untuk karyawan <strong>{{ $item->dataKaryawanBorongan->nama_karyawan }}</strong>?
                                    </div>
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                                        <a href="/hapusGajiSpet/{{ $item->id_gaji_borongan }}" class="btn btn-danger">Hapus</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <!-- Modal Edit Gajian -->
                    <div class="modal fade" id="editGajian-{{ $item->id_gaji_borongan }}" tabindex="-1" aria-labelledby="editGajianLabel" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <form action="/editGajianBoronganSpet/{{ $item->id_gaji_borongan }}" method="POST">
                                    @csrf
                                    <div class="modal-header">
                                    ...
                                    <div class="modal-body">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" class="form-control" id="nama_karyawan" value="{{ $item->dataKaryawanBorongan->nama_karyawan }}" disabled>
                                        </div>
                                        <div class="mb-3">
                                            <label for="bagian" class="form-label">Bagian</label>
                                            <input type="text" class="form-control" id="bagian" value="{{ $item->dataKaryawanBorongan->bagian }}" disabled>
                                        </div>
                                        <div class="mb-3">
                                            <label for="jumlah_s" class="form-label">Jumlah S</label>
                                            <input type="number" step="0.01" class="form-control" id="jumlah_s" name="jumlah_s" value="{{ $item->jumlah_s }}">
                                        </div>
                                        <div class="mb-3">
                                            <label for="jumlah_o" class="form-label">Jumlah O</label>
                                            <input type="number" step="0.01" class="form-control" id="jumlah_o" name="jumlah_o" value="{{ $item->jumlah_o }}">
                                        </div>
                                        <div class="mb-3">
                                            <label for="jumlah_masuk" class="form-label">Jumlah Masuk</label>
                                            <input type="number" step="0.01" class="form-control" id="jumlah_masuk" name="jumlah_masuk" value="{{ $item->jumlah_masuk }}">
                                        </div>
                                        <div class="mb-3">
                                            <label for="jumlah_bonus" class="form-label">Jumlah Bonus</label>
                                            <input type="number" step="0.01" class="form-control" id="jumlah_bonus" name="jumlah_bonus" value="{{ $item->jumlah_bonus }}">
                                        </div>
                                    </div>
                                    <div class="modal-footer">
    ...
    <div class="modal fade" id="tambahAkun" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="staticBackdropLabel">Tambah Gaji Karyawan Borongan</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form action="/tambahGajianBorongan" method="post">
                        @csrf
                        <div class="mt-2">
                        ...
                        <div class="row mt-2 mb-2">
                            <div class="col-md-4">
                                ...
                                <select name="nama_karyawan[]" id="nama_karyawan" class="form-control @error('nama_karyawan') is-invalid @enderror">
                                    ...
                                    @foreach ($karyawan_spet as $k)
                                        <option value="{{ $k->id_karyawan_borongan }}">{{ $k->nama_karyawan }}</option>
                                    @endforeach
                                </select>
                                @error('nama_karyawan')
                            ...
                            <div class="col-md-2">
                                <label for="jumlah_s" class="form-label">Jumlah S:</label>
                                <input type="number" step="0.01" name="jumlah_s[]" class="form-control" placeholder="Jumlah S">
                            </div>
                            <div class="col-md-2">
                                <label for="jumlah_o" class="form-label">Jumlah O:</label>
                                <input type="number" step="0.01" name="jumlah_o[]" class="form-control" placeholder="Jumlah O">
                            </div>
                            <div class="col-md-2">
                                <label for="jumlah_masuk" class="form-label">Jumlah Masuk:</label>
                                <input type="number" step="0.01" name="jumlah_masuk[]" class="form-control" placeholder="Jumlah Masuk">
                            </div>
                            <div class="col-md-2">
                                <label for="jumlah_bonus" class="form-label">Jumlah Bonus:</label>
                                <input type="number" step="0.01" name="jumlah_bonus[]" class="form-control" placeholder="Jumlah Bonus">
                            </div>
                        </div>
                        ...
                        <script>
                            function tambahKaryawan() {
                                var container = document.getElementById('karyawan-container-tambah');
                                var newKaryawan = document.createElement('div');
                                newKaryawan.className = 'row mt-2 mb-2';
                                newKaryawan.innerHTML = `
                                    <div class="col-md-4">
                                        <select name="nama_karyawan[]" class="form-control">
                                            <option value="">Pilih Karyawan</option>
                                            @foreach ($karyawan_spet as $k)
                                                @if (!in_array($k->id_karyawan_borongan, array_merge(old('nama_karyawan', []), request()->input('nama_karyawan', []))))
                                                    <option value="{{ $k->id_karyawan_borongan }}">{{ $k->nama_karyawan }}</option>
                                                @endif
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-md-2">
                                        <input type="number" name="jumlah_s[]" class="form-control" placeholder="Jumlah S">
                                    </div>
                                    <div class="col-md-2">
                                        <input type="number" name="jumlah_o[]" class="form-control" placeholder="Jumlah O">
                                    </div>
                                    <div class="col-md-2">
